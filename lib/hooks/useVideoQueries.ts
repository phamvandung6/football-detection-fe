...
interface UploadVideoOptions {
  onSuccess?: (data: { success: boolean; message: string; videoId?: string }) => void;
  onError?: (error: Error) => void;
  onUploadProgress?: (progress: number) => void;
}

export const useUploadVideo = ({ onSuccess, onError, onUploadProgress }: UploadVideoOptions = {}) => {
  ...
  >({
    mutationFn: (formData: FormData) => uploadVideoFile(formData, onUploadProgress),
    onSuccess: (data) => {
...
export const useDeleteVideo = ({ onSuccess, onError }: DeleteVideoOptions = {}) => {
  const queryClient = useQueryClient();

  return useMutation<void, Error, string>({
    mutationFn: deleteVideo, // Hàm deleteVideo nhận videoId làm đối số
    onSuccess: () => {
      // Invalidate cache của danh sách video
      queryClient.invalidateQueries(['videos'] as InvalidateQueryFilters); 
      // Invalidate cache của video chi tiết nếu cần
      // queryClient.invalidateQueries(['video', videoId]); // Cần videoId ở đây
    ...
    // Có thể dùng onMutate để optimistic update nếu muốn
  });
}; 